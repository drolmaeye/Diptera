    ...
    def axis_validate(self, event, *args):
        if not self.axis.get() == 'More' and not self.axis.get() == 'Custom':
            ...
            if config.stack_choice.get() == ('GPHP' or 'GPHL'):
                ...
                button_320v_curve = Button(frame_pick, text='LH VKB Curvature', width=20, command=lambda: custom_stage.set('16IDB:pm9'))
                button_320v_curve.grid(row=1, column=1, padx=5, pady=5)
                button_320v_elip = Button(frame_pick, text='LH VKB Ellipticity', width=20, command=lambda: custom_stage.set('16IDB:pm10'))
                button_320v_elip.grid(row=2, column=1, padx=5, pady=5)
                button_200v_curve = Button(frame_pick, text='LH HKB Curvature', width=20, command=lambda: custom_stage.set('16IDB:pm7'))
                button_200v_curve.grid(row=1, column=2, padx=5, pady=5)
                button_200v_elip = Button(frame_pick, text='LH HKB Ellipticity', width=20, command=lambda: custom_stage.set('16IDB:pm8'))
                button_200v_elip.grid(row=2, column=2, padx=5, pady=5)
            entry.focus_set()
...
# hard-encoded configuration options
    ...
    mXpco = Device('XPSTEST:m1', pco_args)
