...
class ExpConfigure:
    ...
    def __init__(self, master):
        ...
        self.config_window.geometry('250x360')
    ...
    def confirm_choices(self):
        ...
        valid_list = ['BMB', 'BMDHL', 'GPHP', 'GPHL', 'GPSS', 'IDBLH', 'IDD', 'TEST']
...
class ScanBox:
    ...
    def set_directory(self):
        ...
        elif stack == 'GPHL' or stack == 'GPHP' or stack == 'GPSS' or stack == 'IDBLH':
            zeon_prefix = 'W:\\16idb\\'
        elif stack == 'IDD':
            zeon_prefix = 'IDD TBD'
        else:
    ...
    def axis_validate(self, event, *args):
        if not self.axis.get() == 'More' and not self.axis.get() == 'Custom':
            ...
            popup.geometry('180x400+%d+%d' % (xtop, ytop))
...
def make_trajectory(zero, min, max, velo, motor):
    ...
    elif config.stack_choice.get() == 'GPSS':
    ...
    if motor == mX:
        ...
        if config.stack_choice.get() == 'GPHP' or config.stack_choice.get() == 'GPSS':
            xi = 5
            vi = 6
        else:
            xi = 7
...
# hard-encoded configuration options
    ...
    root.title('Diptera - IDB General Purpose Endstaion (soller slits)')
    # create objects including epics Motors, Struck, etc
    # define 5-motor sample stack
    mX = Motor('XPSGP:m1')
    mY = Motor('XPSGP:m2')
