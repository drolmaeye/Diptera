...
class CoreData:
    ...
    def __init__(self):
        ...
        self.POS = np.linspace(-0.100, 0.100, 11)
        self.POS2 = np.linspace(-0.100, 0.100, 11)
        self.D01 = np.ones(11)
        self.D02 = np.ones(11)
        self.SCA = np.ones(11)
...
class DataLoad:
    ...
    def save_ascii(self):
        ...
        if core.dimension != 1 and not data.slice_flag.get():
            np.savetxt(textfile, core.POS2, fmt='%6.4f', delimiter=', ', header=vertical, footer='\n', comments='')
        np.savetxt(textfile, core.POS, fmt='%6.4f', delimiter=', ', header=horizontal, footer='\n', comments='')
...
class DragVerticalLines:
    ...
    def on_release(self, event):
        ...
        xind = np.abs(core.POS - mid_fin[0]).argmin()
        fly_axis_length = core.POS.shape[0]
...
class Annotate:
    ...
    def on_release(self, event):
        ...
        composite = None
        for y_points in range(len(core.POS2)):
            if box_y_min < core.POS2[y_points] < box_y_max:
                for x_points in range(len(core.POS)):
                    if box_x_min < core.POS[x_points] < box_x_max:
                        distance = ((core.POS2[y_points] - self.y0)**2 + (core.POS[x_points] - self.x0)**2)**0.5
                        if distance < radius:
                            image_index = y_points*len(core.POS) + x_points + 1
...
def onclick(event):
    ...
    x_val = event.xdata
    y_val = event.ydata
    xind = np.abs(core.POS - x_val).argmin()
    yind = np.abs(core.POS2 - y_val).argmin()
    ...
    if image_index in core.dict_points:
        ...
        core.dict_points[image_index] = (core.POS[xind], core.POS2[yind])
...
def update_plot(*args):
    ...
    if core.dimension == 1 or data.slice_flag.get():
        ...
        f_mid = (f_max + f_min)/2
        mid_list = []
        for i in range(core.POS.size-1):
            if not core.SCA[i] < f_mid <= core.SCA[i + 1] and not core.SCA[i + 1] < f_mid <= core.SCA[i]:
                pass
            else:
                f2, f1, x2, x1 = core.SCA[i+1], core.SCA[i], core.POS[i+1], core.POS[i]
                m = (f2 - f1)/(x2 - x1)
                x_mid = (f_mid - f1)/m + x1
        ...
        # ###    x_mid = staff.pv_x0.get()
        if len(mid_list) < 2:
            x_min = np.amin(core.POS)
            x_max = np.amax(core.POS)
            x_span = (x_max - x_min)
        ...
        # index calculator
        yind = data.current_slice.get() - 1
        xind = np.abs(core.POS - x_mid).argmin()
        fly_axis_length = core.POS.shape[0]
        image_index = yind*fly_axis_length + xind + 1
        data.index.set(image_index)
        # ###if staff.focus_flag.get() and counter.data_type.get() == 'Derivative':
        ...
        # ###    x0 = core.POS[guess_index]
        # ###    p0 = [a, x0, 0.5, 0.5, 0.005, 0.005]
        # ###    staff.popt, pcov = curve_fit(piecewise_split_pv, core.POS, core.SCA, p0=p0)
        ...
        for each in array_list:
            if each.plot_flag.get():
                if counter.data_type.get() == 'Derivative':
                    plt.plot(each.POS[1:-1], each.SCA[1:-1], marker='.', ls='-')
                    # ###if staff.focus_flag.get():
                    # ###    plt.plot(core.POS, piecewise_split_pv(core.POS, *staff.popt), 'ro:')
                else:
                    plt.plot(each.POS, each.SCA, marker='.', ls='-')
                    plt.xlim(core.POS[0], right=core.POS[-1])
    else:
        ...
        # plot filled contour
        plt.contourf(core.POS, core.POS2, core.SCA, V, cmap=None)
        plt.colorbar()
        ...
        # work out indices
        xind = np.abs(core.POS - halfx).argmin()
        yind = np.abs(core.POS2 - halfy).argmin()
        data.current_slice.set(yind+1)
        fly_axis_length = core.POS.shape[0]
        image_index = yind*fly_axis_length + xind + 1
        data.index.set(image_index)
        ...
        if data.grid_flag.get():
            eh = plt.gca()
            eh.set_yticks(core.POS2, minor=True)
            eh.set_xticks(core.POS, minor=True)
...
def val_notify(**kwargs):
    # start = time.clock()
    cpt = scan_cpt.get()
    t_pos = now_pos.get()
    core.POS[cpt - 1] = t_pos
    t_cnts1 = now_cnts1.get()
    core.D01[cpt - 1] = t_cnts1
    t_cnts2 = now_cnts2.get()
    core.D02[cpt - 1] = t_cnts2
    # print time.clock() - start


def scan_start_stop(**kwargs):
    flag_val = scan_data_flag.get()
    if flag_val == 0:
        # print caget(now_pos_pv.get())
        # npts = scan_npts.get()
        # print npts
        # i_pos = next_pos.get()
        # print i_pos
        # step_size = scan_step_size.get()
        # for steps in range(0, npts):
        #     positions[steps] = i_pos + steps*step_size
        # print positions
        print 'scan starting!'
    elif flag_val == 1:
        print 'scan is done'
        ...
        # plt.clf()
        # plt.plot(positions, core.D01, marker='.', ls='-')
        # plt.gcf().canvas.draw()


def plot_next_point(**kwargs):
    cpt = kwargs['value']
    if cpt == 0:
        p_zero = next_pos.get()
        npts = scan_npts.get()
        step_size = scan_step_size.get()
        print p_zero
        for points in range(npts):
            positions[points] = p_zero + points*step_size
        # clear fields and plot
        # ###hax.min_pos.set('')
        # ###hax.mid_pos.set('')
        # ###hax.max_pos.set('')
        ...
        start = time.clock()
        # plt.clf().canvas.draw()
        plt.plot(core.POS[cpt-1:cpt], core.SCA[cpt-1:cpt], marker='.', ls='-', color='blue')
        plt.xlim(core.POS[0], right=core.POS[-1])
        plt.gcf().canvas.draw()
        print time.clock() - start
'''
...
next_pos = PV('16TEST1:scan1.P1DV')
scan_npts = PV('16TEST1:scan1.NPTS')
scan_cpt = PV('16TEST1:scan1.CPT', callback=plot_next_point)
scan_step_size = PV('16TEST1:scan1.P1SI')

now_pos = PV('16TEST1:scan1.R1CV')
now_cnts1 = PV('16TEST1:scan1.D01CV')
now_cnts2 = PV('16TEST1:scan1.D02CV')
now_pos_pv = PV('16TEST1:scan1.R1PV')
now_cnts1_pv = PV('16TEST1:scan1.D01PV')
now_cnts2_pv = PV('16TEST1:scan1.D02PV')

scan_val_flag = PV('16TEST1:scan1.VAL', callback=val_notify)
scan_data_flag = PV('16TEST1:scan1.DATA', callback=scan_start_stop)

all_pos = PV('16TEST1:scan1.P1RA')
all_cnts1 = PV('16TEST1:scan1.D01DA')
all_cnts2 = PV('16TEST1:scan1.D02DA')
