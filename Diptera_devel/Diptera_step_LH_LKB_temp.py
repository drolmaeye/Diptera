...
class ExpConfigure:
    ...
    def confirm_choices(self):
        ...
        if stack != 'GPHP' and stack != 'GPHL' and stack != 'IDBLH':
...
# hard-encoded configuration options
    ...
    # 'NAME': [controller, designation, pco, bnc, softGlue, VMAX (in egu/s)]
    stage_dict = {
        'LH xCEN X': ['XPS', mX, mXpco, 's01', 'FI1_Signal', 2.0],
        'LH xCEN Y': ['XPS', mY, mYpco, 's02', 'FI1_Signal', 2.0],
        'LH xSAM Z': ['XPS', mZ, mZpco, 's03', 'FI1_Signal', 2.0],
        'LH xOMEGA': ['XPS', mW, mWpco, 's04', 'FI1_Signal', 20.0]}

    # create lists for drop-down menus
    fly_list = ['LH xCEN Y', 'LH xSAM Z', 'More']
    step_list = ['LH xCEN Y', 'LH xSAM Z', 'More', 'Custom']
